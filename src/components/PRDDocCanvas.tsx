...
export function PRDDocCanvas({
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 overflow-hidden">
      <div className="shrink-0 flex items-center justify-between gap-2 px-4 py-1.5 border-b border-gray-100 dark:border-gray-700">
        {saving && <span className="text-xs text-gray-500 dark:text-gray-400">Saving…</span>}
          ...
          <button
            ...
            className={`px-2.5 py-1 text-xs font-medium rounded transition-colors ${
              viewMode === "preview"
                ...
                : "text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-charcoal dark:hover:text-gray-100"
            }`}
          >
            Preview
          </button>
      ...
      <div ref={scrollContainerRef} className="flex-1 min-h-0 overflow-auto p-4">
        {viewMode === "edit" ? (
          <textarea
            ...
            className="w-full min-h-full font-mono text-sm text-charcoal dark:text-gray-100 placeholder:text-gray-400 dark:placeholder:text-gray-500 border-0 bg-transparent resize-none focus:outline-none focus:ring-0 disabled:opacity-90"
            spellCheck={false}
            rows={20}
          />
        ) : (
          <div className="prd-doc-preview min-h-full text-charcoal text-sm [&_h1]:text-lg [&_h1]:font-bold [&_h1]:mb-2 [&_h2]:text-base [&_h2]:font-semibold [&_h2]:mt-4 [&_h2]:mb-1 [&_h2]:pb-1 [&_h2]:border-b [&_h2]:border-gray-200 [&_h3]:text-sm [&_h3]:font-semibold [&_h3]:mt-3 [&_h3]:mb-1 [&_p]:my-2 [&_ul]:my-2 [&_ul]:list-disc [&_ul]:pl-5 [&_ol]:my-2 [&_ol]:list-decimal [&_ol]:pl-5 [&_li]:my-0.5 [&_strong]:font-bold [&_table]:my-3 [&_table]:w-full [&_table]:border-collapse [&_th]:border [&_th]:border-gray-300 [&_th]:px-3 [&_th]:py-1.5 [&_th]:bg-gray-50 [&_th]:text-left [&_th]:font-semibold [&_td]:border [&_td]:border-gray-300 [&_td]:px-3 [&_td]:py-1.5 [&_tr]:border-b [&_tr]:border-gray-200">
            {isEmpty ? (
              <p className="text-gray-400 dark:text-gray-500">No content yet. Ask the agent to generate a PRD, or switch to Edit to type.</p>
            ) : (
